...
export function N8nImportCard({
  ...
  return (
    <section className="mt-5 rounded-[2rem] bg-white p-6 shadow-soft">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold tracking-tight text-slate-950">n8n Lead-Suche</h2>
          <div className="mt-3 grid gap-2 text-sm text-slate-500 sm:grid-cols-3">
            <span>Konfiguriert: {configured ? "yes" : "no"}</span>
            <span>Callback: /api/imports/osm-leads</span>
            <span>Letzter Import: {lastStatus}</span>
          </div>
